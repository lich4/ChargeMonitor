...
const App = {
    ...
    methods: {
        ...
        get_capacity: function(info) {
            var v = (info.CurrentCapacity / info.NominalChargeCapacity * 100).toFixed(2);
            return v;
        },
        ...
        update_chart_min5: function (opt) {
            ...
            if (!this.chart_min5) {
                ...
                var last_id = 0;
                $("#min5_chart").on("mousewheel", function (e) {
                    var ts = Date.now();
                    if (ts - last_id < 100) { // 频率100ms
                        return;
                    }
                    last_id = ts;
                    var x = Math.round(-e.originalEvent.deltaY);
                    var min_v = (that.min5.length - that.DATA_SPAN) / that.DATA_MOVE_SPAN;
                    if (x != 0) {
                        that.min5_indx += x;
                        if (that.min5_indx > min_v) {
                            that.min5_indx = min_v;
                        } else if (that.min5_indx < 0) {
                            that.min5_indx = 0;
                        }
        ...
        update_chart_hour: function (opt) {
            ...
            if (!this.chart_hour) {
                ...
                var last_id = 0;
                $("#hour_chart").on("mousewheel", function (e) {
                    var ts = Date.now();
                    if (ts - last_id < 100) {
                        return;
                    }
                    last_id = ts;
                    var x = Math.round(-e.originalEvent.deltaY);
                    var min_v = (that.hour.length - that.DATA_SPAN) / that.DATA_MOVE_SPAN;
                    if (x != 0) {
                        that.hour_indx += x;
                        if (that.hour_indx > min_v) {
                            that.hour_indx = min_v;
                        } else if (that.hour_indx < 0) {
                            that.hour_indx = 0;
                        }
        ...
        update_chart_day: function (opt) {
            ...
            if (!this.chart_day) {
                ...
                var last_id = 0;
                $("#day_chart").on("mousewheel", function (e) {
                    var ts = Date.now();
                    if (ts - last_id < 100) {
                        return;
                    }
                    last_id = ts;
                    var x = Math.round(-e.originalEvent.deltaY);
                    var min_v = (that.day.length - that.DATA_SPAN) / that.DATA_MOVE_SPAN;
                    if (x != 0) {
                        that.day_indx += x;
                        if (that.day_indx > min_v) {
                            that.day_indx = min_v;
                        } else if (that.day_indx < 0) {
                            that.day_indx = 0;
                        }
        ...
        update_chart_month: function (opt) {
            ...
            if (!this.chart_month) {
                ...
                var last_id = 0;
                $("#month_chart").on("mousewheel", function (e) {
                    var ts = Date.now();
                    if (ts - last_id < 100) {
                        return;
                    }
                    last_id = ts;
                    var x = Math.round(-e.originalEvent.deltaY);
                    var min_v = (that.month.length - that.DATA_SPAN) / that.DATA_MOVE_SPAN;
                    if (x != 0) {
                        that.month_indx += x;
                        if (that.month_indx > min_v) {
                            that.month_indx = min_v;
                        } else if (that.month_indx < 0) {
                            that.month_indx = 0;
                        }
                        that.update_chart_hour("none");
                    }
    ...
    mounted: function () {
        ...
        setInterval(this.get_all_statistics, 300000); // min5
    }
};
